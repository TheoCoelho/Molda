...
- Straight-line gizmo: `Editor2D` now supports single-segment lines and an optional continuous mode. Segments live inside `__lineMeta` and `applyLineGeometryFromMeta` keeps the Fabric line aligned to those points. The sidebar exposes a toggle for the continuous workflow; when enabled, the next segment starts from the previous endpoint until the user presses Esc, Enter, or double-clicks. Creation shows a live preview and Shift snaps to 45°. When customizing interactions, reuse `ensureLineControls` and wrap mutations with `runWithLineTransformGuard` to avoid double transforms or noisy history entries.
- Bézier pen tool: `Editor2D` persists curves using `__curveMeta` (array of nodes with `anchor`, `handleIn`, `handleOut`, `kind`) and renders them through `computeBezierFromNodes`. The pen UI mirrors Illustrator: click to drop corners, click+drag to pull symmetrical tangents, `Alt` to break handles, `Shift` to snap angles, Enter/double-click to finish, Esc to cancel, and clicking the first node closes the path. Controls are generated on the fly via `syncCurveControls`; wrap metadata mutations with `runWithCurveTransformGuard` so history snapshots stay clean and handle gizmos stay in sync. Moving the object translates anchors/handles through `updateCurveMetaAfterMove`, so reuse that helper whenever you implement custom transforms.
